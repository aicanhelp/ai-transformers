...
class BertClassifierSummarizer(BertPreTrainedModel):
    ...
    def init_encoder_weights(self):
        ext_config = self.config.ext
        if ext_config.param_init != 0.0:
            for p in self.encoder.parameters(): p.data.uniform_(-ext_config.param_init, ext_config.param_init)
        if ext_config.param_init_glorot:
            for p in self.encoder.parameters():
                if p.dim() > 1: xavier_uniform_(p)

    def create_encoder(self):
        return Classifier(self.config.hidden_size)
    ...
    def forward(
        ...
        sents_vec = outputs[torch.arange(outputs.size(0)).unsqueeze(1), labels]
        sents_vec = sents_vec * mask_labels[:, :, None].float()
        sent_scores = self.encoder(sents_vec, mask_labels).squeeze(-1)
        return sent_scores, mask_labels


class TransformerSummarizer(BertClassifierSummarizer):
    def __init__(self, config):
        super().__init__(config)

    def create_encoder(self):
        ext_config = self.config.ext
        return TransformerInterEncoder(self.config.hidden_size, ext_config.ff_size, ext_config.heads,
                                       ext_config.dropout, ext_config.inter_layers)


class RNNSummarizer(BertClassifierSummarizer):
    def __init__(self, config):
        super().__init__(config)

    def create_encoder(self):
        ext_config = self.config.ext
        return RNNEncoder(bidirectional=True, num_layers=1, input_size=self.config.hidden_size,
                          hidden_size=ext_config.rnn_size, dropout=ext_config.dropout)
